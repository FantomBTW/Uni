...
void ReadFile(Club **arr, int *cnt){
	char filename[128];
	...
	Club *newArrClb = (Club*) malloc(newClbsCnt * sizeof(Club));
...
void ShowClubs(Club *clbArr, int cnt){
	if (cnt == 0){
...
void AddClub(Club **arr, int *count) {
  Club new_club;
  ...
  Club *new_arr = (Club*)realloc(*arr, (*count + 1) * sizeof(Club));
  if (new_arr == NULL) {
...
void ChangeClub(Club *arr, int cnt){
	if (cnt == 0){
...
void WriteFile(Club *arr, int cnt){
  if (cnt == 0) {
    printf("\nNothing to save.\n");
    return;
  }
